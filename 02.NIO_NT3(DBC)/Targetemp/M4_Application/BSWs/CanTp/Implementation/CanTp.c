...
#if (  (CANTP_CFG_GEN_MAJOR_VERSION != (5u)) \
...
# define CanTp_Util_RxAssembleNecessaryMetadata(RxHdl, pDataPduPayload, pDataPduMetadata, pSduMetadata, pSduMetadataLength, pFcPduMetadata)    (CanTp_Util_RxAssembleSduMetadataAndFcPduMetadata( (RxHdl), (pDataPduPayload), (pDataPduMetadata), (pSduMetadata), (pSduMetadataLength), (pFcPduMetadata) ));
#else
/* There's no PDU metadata to be assembled. */
# define CanTp_Util_RxAssembleNecessaryMetadata(RxHdl, pDataPduPayload, pDataPduMetadata, pSduMetadata, pSduMetadataLength, pFcPduMetadata)    (CanTp_Util_RxAssembleSduMetadata( (pDataPduPayload), (pSduMetadata), (pSduMetadataLength) ));
#endif
 ...
 *  \details    -
 *  \param[in]  mem1     Pointer to the 1st array. Shall not be null.
 *  \param[in]  mem2     Pointer to the 2nd array. Shall not be null.
 *  \param[in]  length   Length of the arrays.
 *  \return     0u       The arrays are equal.
 *  \return     1u       The arrays are not equal.
 *  \context    TASK|ISR2
 *  \pre        -
 *********************************************************************************************************************/
 ...
 *  \context       TASK|ISR2
 * \reentrant      TRUE for non-overlapping destination memory areas of both SduMetadata and FcPduMetadata and for different RxHdl values.
 ...
 *********************************************************************************************************************/
CANTP_LOCAL_INLINE FUNC(Std_ReturnType, CANTP_CODE) CanTp_Util_RxAssembleSduMetadataAndFcPduMetadata(PduIdType RxHdl,
 ...
 *  \param[in]     pPayloadInfo           PduInfo struct with data-PDU payload. Shall represent a valid buffer.
 *  \param[in]     pMetadataInfo          PduInfo struct with data-PDU metadata. Shall represent a valid buffer.
 ...
 *  \param[in]     pPayloadInfo           PduInfo struct with fc-PDU payload. Shall represent a valid buffer.
 *  \param[in]     pMetadataInfo          PduInfo struct with fc-PDU metadata. Shall represent a valid buffer.
 ...
 *  \details       -
 *  \param[in]     RxPduId        Rx N-PDU id. Shall be less than the size of CanTp_RxPduMap.
 *  \param[in]     FrameType      SingleFrame, FirstFrame, ConsecutiveFrame or FlowControl.
 *  \param[in]     pPayloadInfo   PduInfo struct with N-PDU payload. Shall represent a valid buffer.
 *  \param[in]     pMetadataInfo  PduInfo struct N-PDU metadata. Shall represent a valid buffer.
...
# if (CANTP_RX_OPTIMIZATION != CANTP_OPT_ZERO_SDU)                                                                     /* COV_CANTP_ROBUSTNESS_CASE TX */
  ...
  for (lSduId = 0; lSduId < CanTp_Cfg_GetNumRxSduIds(); lSduId++)
  {
    if (CanTp_RxSduSnvIsAvailable(lSduId))                                                                             /* PRQA S 2741 */ /* MD_CanTp_ExpressionAlwaysTrue */ /* COV_CANTP_POSTBUILD_DELETED TX */
    {
      ...
      /* #35 Assert countermeasure CM_CANTP_CUSTOMADDRCALLOUTFCT */
# if (CANTP_RX_HAS_CUSTOM_ADDRESSING == STD_ON)
      /* Addressing format custom requires a callout to UL */
      if (CanTp_RxCfg_GetAddressingFormat(lSduHdl) == CANTP_ADDRESS_FORMAT_CUSTOM)
      {
        if (!CanTp_IsUpLayerFctsRxUsedOfRxSduCfg(lSduHdl))                                                             /* COV_CANTP_SILENT_RUNTIME_CHECK XF */
        {
          CanTp_DetReportError(CANTP_SID_INIT, CANTP_E_PARAM_CONFIG)
          lResult = FALSE;                                                                                             /* PRQA S 2880 */ /* MD_MSR_Unreachable */
        }
...
# if (CANTP_TX_OPTIMIZATION != CANTP_OPT_ZERO_SDU)                                                                     /* COV_CANTP_ROBUSTNESS_CASE TX */
  ...
  for (lSduId = 0; lSduId < CanTp_Cfg_GetNumTxSduIds(); lSduId++)
  {
    if (CanTp_TxSduSnvIsAvailable(lSduId))                                                                             /* PRQA S 2741 */ /* MD_CanTp_ExpressionAlwaysTrue */ /* COV_CANTP_POSTBUILD_DELETED TX */
    {
      ...
      switch (CanTp_TxCfg_GetAddressingFormat(lSduHdl))
      {
...
#   endif 
      ...
      /* #75 Assert countermeasure CM_CANTP_CUSTOMADDRCALLOUTFCT */
#  if (CANTP_TX_HAS_CUSTOM_ADDRESSING == STD_ON)
      /* Addressing format custom requires a callout to UL */
      if (CanTp_TxCfg_GetAddressingFormat(lSduHdl) == CANTP_ADDRESS_FORMAT_CUSTOM)
      {
        if (!CanTp_IsUpLayerFctsTxUsedOfTxSduCfg(lSduHdl))                                                             /* COV_CANTP_SILENT_RUNTIME_CHECK XF */
        {
          CanTp_DetReportError(CANTP_SID_INIT, CANTP_E_PARAM_CONFIG)
          lResult = FALSE;                                                                                             /* PRQA S 2880 */ /* MD_MSR_Unreachable */
        }
                                                                                                     ...
                                                                                                     P2VAR(uint8, AUTOMATIC, CANTP_APPL_DATA) pFcPduMetadata)
{
  ...
  switch (CanTp_RxCfg_GetAddressingFormat(RxHdl))
  {
...
#  if (CANTP_RX_HAS_CUSTOM_ADDRESSING == STD_ON)
    ...
    case CANTP_ADDRESS_FORMAT_CUSTOM:
    {
      ...
      lResMetaDataOfNSdu.SduLength        = CanTp_RxCfg_GetSduMetadataLength(RxHdl); /* <= CANTP_MAX_SDU_METADATA_LENGTH */
      /* Parameter 4: lResMetaDataOfFcNPdu      - Address of pTpRxState-member FcPduMetadata[] is used as value for this uint8* parameter */
      ...
      lReturnValue = CanTp_RxCfg_GetCustomAddressingCallout(RxHdl)( pDataPduPayload,                  /* P1: Payload as it comes from CanIf. */  /* SBSW_CANTP_CUSTOMADDRCALLOUT_FCTPTR */
      ...
      if (lReturnValue == E_OK)
      {
        ...
        *pSduMetadataLength = CanTp_RxCfg_GetSduMetadataLength(RxHdl);                                                 /* SBSW_CANTP_RXASSEMBLESDUMETADATA_WRITE_PSDUMETADATALENGTH */
...
/* # if (CANTP_RX_HAS_CUSTOM_ADDRESSING == STD_ON) */
  ...
  return lReturnValue;
} /* CanTp_Util_RxAssembleSduMetadataAndFcPduMetadata() */ /* PRQA S 6060 */ /* MD_MSR_STPAR */
                                                                                      ...
                                                                                      P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pMetadataInfo)
{
  ...
  boolean matchResult = TRUE;                                                                                          /* PRQA S 2981 */ /* MD_CanTp_RedundantAssignment */
...
# endif
  ...
  lReturnValue = CanTp_Util_RxAssembleNecessaryMetadata(pTpRxState->RxHdl,                                             /* SBSW_CANTP_RXASSEMBLESDUMETADATA_CALL_PTRS */
                                                        pPayloadInfo,
                                                        pMetadataInfo,
                                                        lSduMetadata,
                                                        &lSduMetadataLength,
...
CANTP_LOCAL_INLINE FUNC(boolean, CANTP_CODE) CanTp_Util_TxSduMetadataFromFcPduMatch(P2CONST(CanTp_TxStateType, AUTOMATIC, CANTP_APPL_DATA) pTpTxState,
                                                                                    ...
                                                                                    P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pMetadataInfo)
{
  boolean result = TRUE;                                                                                               /* PRQA S 2981 */ /* MD_CanTp_RedundantAssignment */
...
#  if (CANTP_TX_HAS_CUSTOM_ADDRESSING == STD_ON)
      case CANTP_ADDRESS_FORMAT_CUSTOM:
      {
        ...
        lResMetaDataOfDataNPdu.SduLength  = CanTp_TxCfg_GetPduMetadataLength(pTpTxState->TxHdl);
        ...
        /* --- Actually conduct the callout --- */
        lReturnValue = CanTp_TxCfg_GetCustomAddressingCallout(pTpTxState->TxHdl)(&lMetaDataOfNSdu,                                  /* P1: Known metadata of NSdu from CanTp_Transmit() call. */ /* SBSW_CANTP_CUSTOMADDRCALLOUT_FCTPTR */
                                                                    ...
                                                                    P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pPdu)
{
  ...
  lAddressingOffset = CanTp_Util_GetAdrOffset(CanTp_RxCfg_GetAddressingFormat(pTpRxState->RxHdl));
...
# if (CANTP_GENERIC_CONNECTIONS == STD_ON)
    if (CanTp_RxCfg_GetSduMetadataLength(pTpRxState->RxHdl) != 0u)
    {
      PduInfoType lPduInfo;
      lPduInfo.SduDataPtr = pTpRxState->SduMetadata;
      lPduInfo.SduLength  = CanTp_RxCfg_GetSduMetadataLength(pTpRxState->RxHdl);
...
} /* CanTp_CanIfRxIndicationSF () */  /* PRQA S 6050 */ /* MD_MSR_STCAL */
...
} /* CanTp_CanIfRxIndicationFF() */ /* PRQA S 6030, 6050 */ /* MD_MSR_STCYC, MD_MSR_STCAL */
...
CANTP_LOCAL_INLINE FUNC(void, CANTP_CODE) CanTp_CanIfRxIndicationFC(CanTp_TxStatePtrType pTpTxState,
                                                                    P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pPdu)
{
  uint8 lAddressingOffset = CanTp_Util_GetAdrOffset(CanTp_TxCfg_GetAddressingFormat(pTpTxState->TxHdl));
...
CANTP_LOCAL_INLINE FUNC(PduIdType, CANTP_CODE) CanTp_Cfg_GetSduHdlByRxPduAddress (PduIdType RxPduId, uint8 FrameType, uint8 Address)
{
  ...
  if (FrameType == CANTP_FRAME_FC)
  {
    ...
    CanTp_TxSduCfgIndIterType lTxSduIndirectionIndex;
    ...
    for (lTxSduIndirectionIndex = (PduIdType)CanTp_GetTxSduCfgIndStartIdxOfRxPduMap(RxPduId);
         lTxSduIndirectionIndex < (PduIdType)CanTp_GetTxSduCfgIndEndIdxOfRxPduMap(RxPduId);
         lTxSduIndirectionIndex++)
    {
      lSduCurrentHdl = (PduIdType)CanTp_GetTxSduCfgInd(lTxSduIndirectionIndex);
  ...
  else
  {
    ...
    /* with an additional address byte in the payload, there may be multiple references */
#if (CANTP_RXSDUCFGIND == STD_ON)                                                                                      /* COV_CANTP_ROBUSTNESS_CASE TX */
    CanTp_RxSduCfgIndIterType lRxSduIndirectionIndex;
    ...
    /* Note: if start and end index are equal, no RxSdu is configured */
    for (lRxSduIndirectionIndex = (PduIdType)CanTp_GetRxSduCfgIndStartIdxOfRxPduMap(RxPduId);
         lRxSduIndirectionIndex < (PduIdType)CanTp_GetRxSduCfgIndEndIdxOfRxPduMap(RxPduId);
         lRxSduIndirectionIndex++)
    {
      lSduCurrentHdl = (PduIdType)CanTp_GetRxSduCfgInd(lRxSduIndirectionIndex);
                                                                                     ...
                                                                                     P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pMetadataInfo)
{
  ...
  if (FrameType == CANTP_FRAME_FC)                                                                                     /* COV_CANTP_PDU_REF */
  {
    ...
    for (lTxSduIndirectionIndex = (PduIdType)CanTp_GetTxSduCfgIndStartIdxOfRxPduMap(RxPduId);
         ...
         lTxSduIndirectionIndex++)
    {
      PduIdType lTmpSduHdl = (PduIdType)CanTp_GetTxSduCfgInd(lTxSduIndirectionIndex);
      P2CONST(CanTp_TxStateType, AUTOMATIC, CANTP_APPL_DATA) pTpTxState = CanTp_TxGetActiveChannelPtr(lTmpSduHdl);
  ...
  else
  {
    ...
    for (lRxSduIndirectionIndex = (PduIdType)CanTp_GetRxSduCfgIndStartIdxOfRxPduMap(RxPduId);
         ...
         lRxSduIndirectionIndex++)
    {
      PduIdType lTmpSduHdl = (PduIdType)CanTp_GetRxSduCfgInd(lRxSduIndirectionIndex);
      P2CONST(CanTp_RxStateType, AUTOMATIC, CANTP_APPL_DATA) pTpRxState = CanTp_RxGetActiveChannelPtr(lTmpSduHdl);
...
CANTP_LOCAL_INLINE FUNC(void, CANTP_CODE) CanTp_RxInitState(CanTp_RxStatePtrType pTpRxState)
{
  ...
  pTpRxState->InitalDLC            = 0u;                                                                               /* SBSW_CANTP_INTERNAL_WRITE_RXSTATE */
                                                                          ...
                                                                          P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) pTpData)
{
  ...
  return CanTp_GetTransmitFctOfLoLayerFcts(lLoLayerIdx)(LoLayerTxPduId, pTpData);                                   /* SBSW_CANTP_LOLAYERTRANSMIT_FCTPTR */
} /* CanTp_LoLayerTransmit */
#endif
...
# endif
  ...
  /* #40 If required, notify PduR of the transmission result. */
# if (CANTP_TX_HAS_CUSTOM_ADDRESSING == STD_ON)
  if (NotifyCode != CANTP_NOTIFY_NORESULT)
# endif
  {
...
# if (CANTP_RX_CHECK_ADDRESSING_FORMAT == STD_ON)
  if (CanTp_Util_GetAdrOffset(CanTp_RxCfg_GetAddressingFormat(pTpRxState->RxHdl)) != 0u)
# endif
  {
...
# if (CANTP_MAX_PDU_METADATA_LENGTH != 0)
    ...
    lPduTxInfo.PduMetadataLength = CanTp_RxCfg_GetPduMetadataLength(pTpRxState->RxHdl);
# endif
...
# if (CANTP_TX_CHECK_ADDRESSING_FORMAT == STD_ON)
  if (CanTp_Util_GetAdrOffset(CanTp_TxCfg_GetAddressingFormat(pTpTxState->TxHdl)) != 0u)
# endif
  {
...
# endif
  ...
  if (lIsChannelStateValid == TRUE)
  {
    ...
    lPduTxInfo.PduMetadataLength = CanTp_TxCfg_GetPduMetadataLength(pTpTxState->TxHdl);
#endif
...
FUNC(void, CANTP_CODE) CanTp_RxIndication(PduIdType RxPduId, P2CONST(PduInfoType, AUTOMATIC, CANTP_APPL_DATA) PduInfoPtr)
{
  ...
  else
  {
    ...
    if ((PduInfoPtr->SduLength - CanTp_Cfg_GetPduMetadataLengthByRxPdu(RxPduId)) > 1u)                                 /* PRQA S 2985 */ /* MD_CanTp_RedundantOperation */
    {
      ...
      {
        lSduHdl = CanTp_Cfg_GetSduHdlByRxPduAndMetadata(RxPduId, lReceivedFrameType, &lPayloadInfo, &lMetadataInfo);   /* SBSW_CANTP_CALL_INTERNAL_RCHECK */
      }
      else
#endif
      {
