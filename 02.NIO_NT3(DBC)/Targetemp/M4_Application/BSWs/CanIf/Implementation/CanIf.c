...
#if (CANIF_CBK_MINOR_VERSION != 0x01u)
...
#if (CANIF_TYPES_MINOR_VERSION != 0x25u)
...
#if (CANIF_API_MINOR_VERSION != 0x04u)
...
/* Lookup table to convert the states from Can_ControllerStateType to the modes from CanIf_ControllerModeType.
    The values of the states from Can_ControllerStateType are explicit defined in ASR 4.4.0 specification of CAN driver.
...
CANIF_LOCAL_INLINE FUNC(void, CANIF_CODE) CanIf_InitSubInitFifoQueue(void)
{
  ...
  for(counter = 0; counter < CanIf_GetSizeOfTxQueueIndex2FifoEl(); counter++) /* FETA_CANIF_X */
  {
  ...
  for(counter = 0; counter < CanIf_GetSizeOfTxFifoQueueBase(); counter++) /* FETA_CANIF_X */
  {
...
# else
        ...
        for (MappedTxPdusIdx = CANIF_CFG_TXBUFFERPRIOBYCANIDBYTEQUEUE_STARTIDX2MAPPEDTXPDUS(txBufferCfgIdx); MappedTxPdusIdx < CANIF_CFG_TXBUFFERPRIOBYCANIDBYTEQUEUE_STOPIDX2MAPPEDTXPDUS(txBufferCfgIdx); MappedTxPdusIdx++) /* FETA_CANIF_12 */
        {
...
CANIF_LOCAL_INLINE FUNC(Std_ReturnType, CANIF_CODE) CanIf_InitSubCheckJ1939AddrTables(void)
{
  ...
  for (controllerId = 0u; controllerId < CANIF_CFG_MAX_CONTROLLER; controllerId++) /* FETA_CANIF_X */
  {
...
# endif
...
#endif
  {
    ...
    PduIdType CanPduId;
    for(CanPduId = 0; CanPduId < CANIF_CFG_MAX_TXPDUS; CanPduId++) /* FETA_CANIF_X */
    {
    ...
    for (controllerId = 0u; controllerId < CANIF_CFG_MAX_CONTROLLER; controllerId++) /* FETA_CANIF_X */
    {
...
#if (CANIF_J1939_DYN_ADDR_SUPPORT != CANIF_J1939_DYN_ADDR_DISABLED)
      {
        ...
        if(CANIF_CFG_CTRLJ1939DYNADDRSUPPORT(controllerId) != CANIF_J1939_DYN_ADDR_DISABLED ) /* COV_CANIF_NOT_APPLICABLE_FOR_EACH_CONFIG */
        {
          for(index = 0; index < (CANIF_J1939_CTRL_ADDR_TBL_SIZE - 1u); index++) /* COV_CANIF_NOT_APPLICABLE_FOR_EACH_CONFIG */ /* FETA_CANIF_Y */
          {
...
# endif
...
#if (CANIF_SET_PDU_RECEPTION_MODE_SUPPORT == STD_ON)
    {
      ...
      for(index = 0; index < CanIf_GetSizeOfPduRxMode(); index++) /* FETA_CANIF_X */
      {
...
CANIF_LOCAL_INLINE FUNC(void, CANIF_CODE) CanIf_RxIndicationSubBinarySearch(P2VAR(CanIf_RxSearchParamsType, AUTOMATIC, AUTOMATIC) rxSearchParams)
{
  ...
  while (firstRxPduId <= lastRxPduId) /* PRQA S 0771 */ /* MD_CanIf_MultipleBreak */ /* FETA_CANIF_14 */
  {
...
#  else
      ...
      if (CanIf_GetTxQueueCounter(txBufferPrioByCanIdBaseIdx) != 0u)
      {
        ...
        for (idx2MappedTxPdus = ((CanIf_AnyIdxType)CANIF_CFG_TXBUFFERPRIOBYCANIDBYTEQUEUE_STOPIDX2MAPPEDTXPDUS(txBufferCfgIdx)-1u); idx2MappedTxPdus >= CANIF_CFG_TXBUFFERPRIOBYCANIDBYTEQUEUE_STARTIDX2MAPPEDTXPDUS(txBufferCfgIdx); idx2MappedTxPdus--) /* FETA_CANIF_13 */
        {
...
# endif
  ...
  for (controllerId = 0u; controllerId < (sizeof(CanIf_CtrlStates) / sizeof(CanIf_CtrlStatesType)); controllerId++) /* FETA_CANIF_Z */
  {
...
# endif
  {
    ...
    for (numWUsrc = 0u; numWUsrc < CANIF_CFG_MAX_WAKEUPSOURCES; numWUsrc++) /* FETA_CANIF_X */
    {
...
\ID FETA_CANIF_1
\DESCRIPTION The for loop iterates over all configured Tx-Buffers, which are assigned to a CAN controller.
             The number of iteration is limited and statically given by the number of configured Tx-Buffers
             at a CAN controller.

\COUNTERMEASURE \N The number of iteration is therefore not influencend by the processing process (push, pop and cancel Tx-PDUs)
                   of the Tx-Buffers.

\ID FETA_CANIF_2
\DESCRIPTION The while loop iterates over the currently queued Tx-PDUs in the Tx-Buffer to clear their cancel settings.
             The number of iteration depends on the currently number of queued Tx-PDUs in the Tx-Buffer.

\COUNTERMEASURE \N The while loop is locked by CANIF_EXCLUSIVE_AREA_1 to prevent push, pop and cancel Tx-PDUs while processing the
                   Tx-Buffer. This ensures a termination of the loop.
...
\COUNTERMEASURE \D DSGN-CanIfDiag2274
...
\DESCRIPTION The while loop iterates over the currently queued Tx-PDUs in the Tx-Buffer to find the next not canceled Tx-PDU.
             The number of iteration depends on the currently number of the queued Tx-PDUs in the Tx-Buffer.

\COUNTERMEASURE \N The while loop is locked by CANIF_EXCLUSIVE_AREA_2 to prevent push, pop and cancel Tx-PDUs while processing the
                   Tx-Buffer. This ensures a termination of the loop.

\ID FETA_CANIF_6
\DESCRIPTION The for loop iterates over all configured queue flags (each flag repesent one Tx-PDU that can be queued) of the
             Tx-Buffer to clear them.
             The number of iteration is limited and statically given by the number of configured queue flags of the Tx-Buffer.
\COUNTERMEASURE \N The number of iteration is therefore not influencend by the processing process (push, pop and cancel Tx-PDUs)
                   of the Tx-Buffer.
...
\COUNTERMEASURE \N The variable PduInfoPtr->SduLength (value 0 - 64) is checked (>=) against CANIF_CFG_TX_TXMETADATALENGTH(localPduPtr->swPduHandle) (value 1 -4).
                   Therefore the value for PduInfoPtr->SduLength is ensured to be between 1 and 64. The value for startPos is between 0 and 63 after the calaculation.
                   The variable endPos is first set to PduInfoPtr->SduLength (value 1 - 64, regarding to check above).
                   The value for endPos is between 0 and 60 after the calaculation.
                   Therefore the for loop terminates always after between 0 and 3 iterations.

\ID FETA_CANIF_8
\DESCRIPTION The for loop termination is determined by a value returned by an parameterized GenData access.
\COUNTERMEASURE \N The argument for the parameterized GenData access is checked by the caller for a valid access to the table CanIf_RxPduConfig[].
                   Therefore it is a strictly monotonic for loop.

\ID FETA_CANIF_9
\DESCRIPTION The for loop iterates over the currently queued Tx-PDUs in the Tx-Buffer to find the Tx-PDU with the highest priority.
             The number of iteration depends on the number and the priority of the currently queued Tx-PDUs in the Tx-Buffer.
\COUNTERMEASURE \N The for loop is locked by CANIF_EXCLUSIVE_AREA_2 to prevent push, pop and cancel Tx-PDUs while processing the
                   Tx-Buffer. This ensures a termination of the loop.
...
\ID FETA_CANIF_12
\DESCRIPTION The for loop iterates over all configured queue states (each state repesent one Tx-PDU that can be queued) of the
             Tx-Buffer to clear them.
             The number of iteration is limited and statically given by the number of configured queue states of the Tx-Buffer.
\COUNTERMEASURE \N The number of iteration is therefore not influencend by the processing process (push, pop and cancel Tx-PDUs)
                   of the Tx-Buffer.
...
\ID FETA_CANIF_X
\DESCRIPTION The for loop termination is determined by a value returned by an static GenData access.
\COUNTERMEASURE \N The GenData are accessed via the CanIf_ConfigDataPtr. The CanIf_ConfigDataPtr is set and cheked in CanIf_Init().
                   This ensures a valid GenData access and a termination of the loop (strictly monotonic for loop).

\ID FETA_CANIF_Y
\DESCRIPTION The for loop termination is determined by a value from the calculation
             CANIF_J1939_CTRL_ADDR_TBL_SIZE - 1u .
\COUNTERMEASURE \N CANIF_J1939_CTRL_ADDR_TBL_SIZE is statically defined to 256 in CanIf.c.
                   Therefore the for loop terminates always after 255 iterations (strictly monotonic for loop).

\ID FETA_CANIF_Z
\DESCRIPTION The for loop termination is determined by a value from the calculation
             sizeof(CanIf_CtrlStates) / sizeof(CanIf_CtrlStatesType) .
\COUNTERMEASURE \N The number of iteration is limited by sizeof of corresponding type and corresponding variable
                   (strictly monotonic for loop).
